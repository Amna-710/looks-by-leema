...
export function getDatabaseUrl() {
  ...
  console.log('[booking-email] Using RTDB URL', url.replace(/\/\/[^@]+@/, '//***@'));
  return url;
}
...
export async function getBooking(bookingId) {
  const url = bookingsUrl(bookingId);
  console.log('[booking-email] Fetching booking', { bookingId, url });
...
export async function patchBooking(bookingId, fields) {
  const url = bookingsUrl(bookingId);
  console.log('[booking-email] Patching booking', { bookingId, fields });
